...
const changeIsActive = (index: number) => {
  ...
  const childItem = store.state.workspace.lastActiveItem;
  if (
    itemList[index].defaultChild != null &&
    itemList[index].defaultChild != undefined &&
    (childItem == null || childItem == undefined)
  ) {
    ...
    debugger
    const childPath1 = router.options;
    const childPath4 = router.options.routes;
    const childPath5 = router.options.routes[0];
    const childPath2 = router.options.routes[lastActiveIndex.value];
    const childPath3 = router.options.routes[lastActiveIndex].children[childItem];
    const childPath = router.options.routes[lastActiveIndex].children[childItem].path;
    router.push(childPath);
  }
};

const refresh = () => {
  const tree = route.meta.indexTree;
  var array = tree.split("_");
  store.commit("changeIsActive", array[0]);
  changeIsActive(array[0]);
};
refresh();
</script>
