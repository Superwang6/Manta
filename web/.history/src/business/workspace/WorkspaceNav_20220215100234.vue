...
const lastActiveIndex = computed(() => store.state.workspace.lastActiveItem);
...
//   if (array.length > 0) {
//     store.commit("changeChildIsActive", array[1]);
//     changeIsActive(array[1]);
//   }
// };
// refresh();
onMounted(() => {
  const lastActiveItem = store.state.workspace.lastActiveItem;
  changeIsActive(lastActiveItem.value);
})
</script>
